...
var http        = require("http"),
    ...
    brckDist    = {root: path.join(__dirname, "..", "dist")},
    //zipped      = { ".js": "application/javascript", ".css": "text/css"},
    defaultPort = 6800;

if (require('fs').existsSync(brckDist.root) === false) {
    brckDist    = {root: path.join(__dirname, "..", "src")};
}
...
function attachStatic(inst) {
    var srv     = inst.httpServer,
        ...
        nlsDir  = { root: path.join(inst.supportDir, "nls")};
    ...
    srv.on("request", function(req, res) {
        ...
        } else if (req.url.startsWith("/support/nls/")) {
            try {
                return send(req, req.url.substr("/support/nls".length), nlsDir).pipe(res);
            } catch (e) {
                res.writeHead(500, {
                    "Content-Length": e.message.length,
                    "Content-Type": "text/plain"
...
Server.prototype.start = function(srv, opts) {
    switch (typeof srv) {
        case "undefined":
        case "null":
    ...
    this.defaultNls = path.join(brckDist.root, "nls");
